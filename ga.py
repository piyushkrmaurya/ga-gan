...
class GeneticAlgorithm:
    ...
    def parent_generator1(self):
        while True:
            ...
            indices = np.random.choice(
                np.arange(len(self.population)), 2, p=p, replace=False
            )
    ...
    def parent_generator(self):
        ...
        while True:
            tournament_size = 10
            indices = np.random.choice(
                np.arange(len(self.fitness)), tournament_size, replace=False
            )
...
class LineGA(GeneticAlgorithm):
    ...
    def mutate(self, population, probability=0.01):
